...
func TestHTTPRedirectHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"subdomain redirect",
			"test-sub-12345678.t.hostbetter.live",
			"/foo",
			http.StatusMovedPermanently,
			"https://test-sub-12345678.t.hostbetter.live/foo",
		},
		{
...
func TestRedirectToWarningPage(t *testing.T) {
	...
	r := httptest.NewRequest("GET", "https://happy-tiger-abcdef01.t.hostbetter.live/path?q=1", nil)
	r.Host = "happy-tiger-abcdef01.t.hostbetter.live"
	w := httptest.NewRecorder()
	...
	loc := w.Header().Get("Location")
	if !strings.HasPrefix(loc, "https://t.hostbetter.live/#/warning?") {
		t.Errorf("Location = %q, want prefix https://t.hostbetter.live/#/warning?", loc)
	}
	if !strings.Contains(loc, "redirect="+url.QueryEscape("https://happy-tiger-abcdef01.t.hostbetter.live/path?q=1")) {
		t.Errorf("Location missing redirect param: %q", loc)
	}
	if !strings.Contains(loc, "subdomain="+url.QueryEscape("happy-tiger-abcdef01.t.hostbetter.live")) {
