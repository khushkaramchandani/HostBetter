# t.hostbetter.live Architecture

## Overview

t.hostbetter.live is a minimal SSH tunneling service that exposes local applications to the internet via random subdomains with automatic SSL.

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│                              t.hostbetter.live SERVER                                │
│                                                                             │
│  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐  ┌─────────────┐   │
...
1. Client connects: `ssh -t -R 80:localhost:8080 t.hostbetter.live`
...
1. Extract subdomain from `Host` header (e.g., `happy-tiger-a1b2c3d4.t.hostbetter.live`)
...
type Server struct {
    ...
    domain        string                     // Configurable domain (default: t.hostbetter.live)
...
| `TLS_CERT` | `/etc/letsencrypt/live/t.hostbetter.live/fullchain.pem` | TLS certificate |
| `TLS_KEY` | `/etc/letsencrypt/live/t.hostbetter.live/privkey.pem` | TLS private key |
